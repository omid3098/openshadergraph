...
namespace OpenShaderGraph.Tests.Core.Logic
{
    ...
    public class GraphManagerNodeTests
    {
        ...
        public void RemoveNode_NodeExists_RemovesNodeAndConnections()
        {
            ...
            Assert.AreEqual(0, _graphData.GetConnections().Count);
            Assert.IsTrue(nodeRemovedFired);
            Assert.AreSame(node1, removedNode);
        }
        ...
        public void RemoveNode_NodeDoesNotExist_DoesNothing()
        {
            ...
            // Assert
            Assert.AreEqual(initialNodeCount, _graphData.GetNodes().Count);
        }
        ...
        public void DuplicateNode_NodeExists_AddsClonedNode()
        {
            ...
            // Assert
            Assert.AreEqual(initialNodeCount + 1, _graphData.GetNodes().Count);
            Assert.IsNotNull(addedNode);
            Assert.AreNotSame(node1, addedNode);
            Assert.AreEqual(node1.GetName(), addedNode.GetName());
            Assert.AreEqual(new Vector2(130, 130), addedNode.GetPosition());
        }
    }
}