...
func validate_connection(connection: ConnectionData) -> bool:
    ...
    if connection.from_pin.direction == connection.to_pin.direction:
        Logger.log("[BaseGraphData] Connection from and to the same pin: " + connection.from_pin.name)
        return false
    # Node has no output pin
    if connection.from_node.outputs.size() == 0:
        Logger.log("[BaseGraphData] Node has no output pin: " + connection.from_node.name)
        return false
    # Node has no input pin
    if connection.to_node.inputs.size() == 0:
        Logger.log("[BaseGraphData] Node has no input pin: " + connection.to_node.name)
        return false
    # Node source does not exist in the graph
    if not nodes.has(connection.from_node):
        Logger.log("[BaseGraphData] Node does not exist in the graph: " + connection.from_node.name)
        return false
    # Node destination does not exist in the graph
    if not nodes.has(connection.to_node):
        Logger.log("[BaseGraphData] Node does not exist in the graph: " + connection.to_node.name)
        return false
    ...
    if connection.from_pin.data_type != connection.to_pin.data_type:
        Logger.log("[BaseGraphData] Type mismatch: " + connection.from_pin.data_type + " -> " + connection.to_pin.data_type)
        return false
    return true