...
func test_complete_shader_graph_workflow():
	...
	assert_equal("New Graph", graph.get_name(), "Graph should have default name")
	
	# Create nodes for a simple shader (Constant -> Add -> Output)
	var color_pin = PinData.new("color", "vector3", PinData.PinType.OUTPUT, Vector3.ZERO)
	var value_pin = PinData.new("value", "float", PinData.PinType.OUTPUT, 0.0)
	var input1_pin = PinData.new("input1", "vector3", PinData.PinType.INPUT, Vector3.ZERO)
	var input2_pin = PinData.new("input2", "vector3", PinData.PinType.INPUT, Vector3.ZERO) # Changed to vector3 to create type mismatch with float
	var result_pin = PinData.new("result", "vector3", PinData.PinType.OUTPUT, Vector3.ZERO)
	var albedo_pin = PinData.new("albedo", "vector3", PinData.PinType.INPUT, Vector3.ZERO)
	...
	assert_equal(4, graph.get_nodes().size(), "Graph should have 4 nodes")
	...
	assert_equal(2, graph.get_connections().size(), "Graph should have 2 valid connections")
...
func test_multi_graph_workflow():
	...
	var graph1: BaseGraphData = graph_manager.create_new_graph("Shader Graph 1")
	
	var graph2: BaseGraphData = graph_manager.create_new_graph("Shader Graph 2")
	
	var graph3: BaseGraphData = graph_manager.create_new_graph("Shader Graph 3")
...
func test_complex_connection_validation():
	...
	var input_out = PinData.new("value", "float", PinData.PinType.OUTPUT, 0.0)
	var math1_in = PinData.new("a", "float", PinData.PinType.INPUT, 0.0)
	var math1_out = PinData.new("result", "float", PinData.PinType.OUTPUT, 0.0)
	var math2_in = PinData.new("b", "float", PinData.PinType.INPUT, 0.0)
	var math2_out = PinData.new("result", "float", PinData.PinType.OUTPUT, 0.0)
	var output_in = PinData.new("final", "float", PinData.PinType.INPUT, 0.0)
	...
	assert_equal(3, graph.get_connections().size(), "All connections should be valid")
	...
	assert_equal(3, graph.get_connections().size(), "Invalid connections should be rejected")

# Test graph data integrity during operations
func test_graph_data_integrity():
	var graph = graph_manager.create_new_graph()
	...
	assert_equal("Node1", graph.get_nodes()[0].get_name(), "First node name should be preserved")
	assert_equal("Type1", graph.get_nodes()[0].get_type(), "First node type should be preserved")
	assert_equal(Vector2(10, 20), graph.get_nodes()[0].get_position(), "First node position should be preserved")
	
	assert_equal("Node2", graph.get_nodes()[1].get_name(), "Second node name should be preserved")
	assert_equal("Type2", graph.get_nodes()[1].get_type(), "Second node type should be preserved")
	...
	assert_equal("Modified Node1", graph.get_nodes()[0].get_name(), "Modified node name should persist")
	assert_equal(Vector2(100, 200), graph.get_nodes()[0].get_position(), "Modified node position should persist")
...
func test_event_driven_architecture():
	...
	var external_handler = func(graph: BaseGraphData):
		external_listener_calls.append(graph.get_name())
	...
	var graph1: BaseGraphData = graph_manager.create_new_graph("External Test Graph 1")
	
	var graph2: BaseGraphData = graph_manager.create_new_graph("External Test Graph 2")
...
func test_error_recovery():
	var graph: BaseGraphData = graph_manager.create_new_graph()
	
	# Test operations on empty graph
	assert_equal(0, graph.get_nodes().size(), "Empty graph should have no nodes")
	assert_equal(0, graph.get_connections().size(), "Empty graph should have no connections")
	...
	var phantom_pin1 = PinData.new("out", "float", PinData.PinType.OUTPUT, 0.0)
	var phantom_pin2 = PinData.new("in", "float", PinData.PinType.INPUT, 0.0)
	...
	assert_equal(0, graph.get_connections().size(), "Connection with non-existent nodes should be rejected")
	...
	assert_equal(1, graph.get_nodes().size(), "Graph should remain functional after null operations")
...
func test_larger_dataset_performance():
	...
	# Create many nodes
	for i in range(100):
		var output_pin = PinData.new("out_%d" % i, "float", PinData.PinType.OUTPUT, 0.0)
	...
	assert_equal(100, graph.get_nodes().size(), "Should have 100 nodes")
...
func test_different_graph_types():
	...
	for graph in graphs:
		...
		assert_equal(0, graph.get_nodes().size(), "Graph should start empty")
		...
		assert_equal(1, graph.get_nodes().size(), "Each graph type should support adding nodes")
		
		# Test basic properties
		assert_not_null(graph.get_name(), "Graph should have a name")
		assert_not_null(graph.get_graph_type(), "Graph should have a type")
		assert_equal(0, graph.get_connections().size(), "New graph should have no connections")