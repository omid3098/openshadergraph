...
async function readExampleGraphs(): Promise<Array<{ key: string; graph: Graph }>> {
  const result: Array<{ key: string; graph: Graph }> = [];
  ...
  // Programmatic examples mirroring server registry
  const programmatic = [
    {
...
function ensureSurface(graph: Graph): Graph {
...
async function main() {
  try {
    ...
    const graphs = await readExampleGraphs();
