...
import {
  ...
  type NodeTemplate,
} from "@/core/schema/nodes";
import { buildRFNodeFromTemplate } from "@/core/ui/nodeFactory";
import { isAbortError } from "@/lib/errors";
import { prepareVisibleNodes } from "@/core/ui/visible";
import {
  groupSelected as utilGroupSelected,
  ungroupGroup as utilUngroupGroup,
} from "@/core/graph/grouping";
...
export function NodeEditor({ onChange }: NodeEditorProps) {
  ...
  // Load node palette for context menu
  useEffect(() => {
  ...
  // Derived graph view for nested groups
  const currentParentId = viewPath.length
  ...
  // Updater helpers for node data
  const updateNodeInputValue = useCallback(
    (id: string, pinId: number, next: number[] | string | number) => {
      setNodes((prev) =>
        prev.map((n) => {
          if (n.id !== id) return n;
          const tpl = (n.data as any)?.template;
          if (!tpl || !Array.isArray(tpl.inputs)) return n;
          const idx = tpl.inputs.findIndex(
            (p: any, i: number) => (typeof p.id === "number" ? p.id === pinId : i === pinId),
          );
          if (idx < 0) return n;
          ...
          const nextTpl = {
            ...tpl,
            inputs: tpl.inputs.map((p: any, i: number) => (i === idx ? { ...p, value: normalized } : p)),
          };
    ...
    (id: string, label: string) => {
      setNodes((prev) =>
        prev.map((n) => {
          ...
          return {
            ...n,
            data: { ...(n.data as any), label, template: nextTpl },
          } as any;
        }),
      );
    },
  ...
  const addNodeMeta = useCallback(
    (id: string, metaKey: string) => {
      if (!metaKey) return;
      setNodes((prev) =>
        prev.map((n) => {
          if (n.id !== id) return n;
          const tpl = (n.data as any)?.template ?? {};
          const meta: string[] = Array.isArray((tpl as any).meta)
            ? ([...(tpl as any).meta] as string[])
            : [];
  ...
  const removeNodeMeta = useCallback(
    (id: string, metaKey: string) => {
      setNodes((prev) =>
        prev.map((n) => {
          if (n.id !== id) return n;
          const tpl = (n.data as any)?.template ?? {};
          const meta: string[] = (
            Array.isArray((tpl as any).meta) ? (tpl as any).meta : []
          ).filter((m: any) => m !== metaKey);
          return { ...n, data: { ...(n.data as any), template: { ...tpl, meta } } } as any;
        }),
      );
    },
  ...
  const loadExampleGraph = useCallback(
    async (ex: { key: string; label: string }) => {
      try {
        ...
        const fragmentPass = (graph.nodes ?? []).find(
          (n) => n.type === "fragment_pass",
        );
        const vertexPass = (graph.nodes ?? []).find(
          (n) => n.type === "vertex_pass",
        );
    ...
    [setNodes, setEdges, updateNodeInputValue, updateNodeLabel, addNodeMeta, removeNodeMeta],
  );

  // Fetch examples and load first by default
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!res.ok) throw new Error(String(res.status));
        const data = await res.json();
        const list: Array<{ key: string; label: string }> = Array.isArray(
          data.examples,
        )
          ? data.examples
          : [];
  ...
  const deleteNodeById = (id: string) => {
    ...
    if (node && (node as any).deletable === false) return;
  ...
  return (
    <div className="w-full h-full relative">
      ...
      <div className="absolute left-2 top-2 z-10 flex items-center gap-2 text-xs bg-background/80 backdrop-blur px-2 py-1 rounded-md border">
        ...
        <button className="hover:underline" onClick={() => setViewPath([])}>
          {graphName}
        </button>
        {viewPath.map((id, i) => {
            ...
            <span key={id} className="flex items-center gap-1">
              ...
              {isLast ? (
                <span className="font-medium">
                  {(n?.data as any)?.label ?? (n?.data as any)?.type ?? id}
                </span>
              ) : (
                <button
